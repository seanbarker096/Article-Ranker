...
const CommentList = ({ articleId }) => {
  ...
  useEffect(() => {
    if (deleteCommentReqStatus === "idle" && commentStateRef.current.clicked) {
      dispatch(deleteComment({ resourceId: commentStateRef.current.id }));
      registerDeleteClick({ clicked: false, id: null });
    }
  }, [commentToDelete, deleteCommentReqStatus, dispatch, registerDeleteClick]);
  ...
  const renderComments = () => {
    if (comments) {
      return Object.values(comments).map((comment) => {
        ...
        if (deleteCommentErr && deleteCommentErr.resourceId === comment._id) {
