...
const ArticleForm = ({
  ...
  const renderError = (formError) => AjaxForm.renderError(formError);
  ...
  return (
    <div>
      {renderError()}
      <form onFocus={handleFocus} className="ui form" onSubmit={handleSubmit}>
        {renderFields()}
        <button className="ui button primary">Submit</button>
        <button onClick={handleCancelClick} className="ui button secondary">
          Cancel
        </button>
      </form>
    </div>
