...
// const jwtRefreshExpiryTime = '86400s';
...
// const createTokensAndCookies = async (req, res, next, currentRefreshToken) => {
//     const promises = [];
//     promises.push(jwtToken.sign({ id: req.user._id }));
//     promises.push(csrfToken.create());
//     if (!currentRefreshToken) promises.push(jwtRefreshToken.create(req.user));
...
//     if (!currentRefreshToken) jwtRefreshCookie.create(req, res, tokens[2]);
// };
...
//     //this avoids us generating and storing them unneccessarily
//     let refreshToken;
//     //if token exists check it is still valid else we issue a new one
//     if (req.cookies.jwtRefresh) {
//         //returns true  if current is valid, otherwise returns new token
//         refreshToken = await jwtRefreshToken.verifyRefreshToken(
//             req.cookies.jwtRefresh,
//             true
//         );
//     }
//     req.user = user;
//     await createTokensAndCookies(req, res, next, refreshToken);
...
//     console.log('user', refreshToken.owner);
...
//     //when request comes in call passport.auth which returns funciton, which we then call immediately with req, res, next
...
//         },
//         async (err, user, info) => {
//             //set req.user to create tokens
//             req.user = user;
//             await createTokensAndCookies(req, res, next);
//             res.status(200).json({
//                 status: 'success',
//             });
//         }
